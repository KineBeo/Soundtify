...
export class ArtistService {
    ...
    async getRandomArtistByCount(count: number) {
        if (count > 30) {
            throw new BadRequestException('You can only request a maximum of 30 random artists at a time.');
        }
        
        if (count < 0) {
            throw new BadRequestException('Please provide a positive number of artists to retrieve.');
        }

        const maxCount = Math.min(count, 30); // Limit the count to a maximum of 30
        return await this.artistRepository.createQueryBuilder()
            .orderBy('RANDOM()')
            .limit(maxCount)
            .getMany();
    }
